...
const Allogaz = () => {
  ...
  // Filter only allogaz trucks
  const allogazTrucks = trucks.filter(truck => truck.truckType === 'allogaz');
  ...
  const handleSupplyTruck = () => {
    if (!selectedTruckId || !selectedDriverId) {
      toast({
        ...
        description: "Veuillez sélectionner un allogaz et un chauffeur",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleReturnTruck = () => {
    ...
    if (!latestSupplyOrder) {
      toast({
        ...
        description: "Aucun bon de sortie trouvé pour ce chauffeur. Veuillez d'abord alimenter l'allogaz.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Gestion des Allogaz</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Sélection de l'Allogaz et du Chauffeur
          </CardTitle>
        </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedTruckId} onValueChange={setSelectedTruckId}>
                <SelectTrigger id="truck-select">
                  <SelectValue placeholder="Sélectionner un allogaz" />
                </SelectTrigger>
                <SelectContent>
                  {allogazTrucks.length === 0 && (
                    <SelectItem disabled value="none">Aucun allogaz disponible</SelectItem>
                  )}
                  {allogazTrucks.map(truck => {
